...
def load_data():
    ...
    with open('app/api_service/my_upload/9intents.json') as json_file:
        ...
        for i in range(len(keys)):
            ...
            combine_with = values[i]['combine_with']
            how_to_use_skill = values[i]['how_to_use_skill']
            introduce = values[i]['introduce']
            print(type(name), type(build_item), type(support_socket), type(counter))
            champion = Champion(id=str(id),name=name, build_item=build_item, support_socket=support_socket, counter=counter, be_countered=be_countered, skill_up=skill_up, how_to_play=how_to_play, combo=combo, combine_with=combine_with, how_to_use_skill=how_to_use_skill, introduce=introduce)
