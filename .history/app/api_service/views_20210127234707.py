...
class ConversationView(MethodView):
    ...
    def post(self):
        ...
        ans['prob'] = str(prob)
        if hero != "":
            ans['champion'] = str(hero)
            entities['champion'] = str(hero)
        if skill != "":
            ans['skill'] = str(skill)
        ...
        intent=str(dict_response['intent']),
        action=str(dict_response['action']), 
        entities=str(entities))
        print(dict_response['message_answer'])
        if intent == 'support_socket':
            dict_response['message_answer'] = get_response_image(dict_response['message_answer'])
